...
async function cleanupBatch() {
    ...
    const deletedKeys = new Set((response.Deleted ?? [])
        .map((item) => item.Key)
        .filter((key) => Boolean(key)));
    if (response.Errors?.length) {
        for (const error of response.Errors) {
            console.error(`Failed to delete S3 object ${error.Key ?? "<unknown>"}: ${error.Code ?? "Unknown"} ${error.Message ?? ""}`.trim());
        }
    }
    const deletedScreenshots = screenshots.filter((screenshot) => deletedKeys.has(screenshot.s3Key));
    if (!deletedScreenshots.length) {
        console.warn(`No screenshots were removed from S3 for the current batch ending at ${screenshots.at(-1)?.capturedAt.toISOString()}.`);
        return 0;
    }
    ...
    if (failedCount > 0) {
        console.warn(`Skipped deleting ${failedCount} screenshot records because their S3 objects were not removed.`);
    }
...
try {
    ...
    console.log(`Screenshot cleanup complete. Removed ${deletedCount} screenshots older than ${cutoff.toISOString()}.`);
}
finally {
    await prisma.$disconnect();
}
//# sourceMappingURL=cleanup-screenshots.js.map