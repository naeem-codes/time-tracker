...
app.setErrorHandler((error, request, reply) => {
    ...
    if (error instanceof z.ZodError) {
        return reply.code(400).send({
            message: error.issues[0]?.message ?? "Please check the information provided",
        });
    }
...
function currentWorkDate(timezone, at = new Date()) {
...
function startOfWorkDate(workDate, timezone) {
    ...
    const firstPass = new Date(utcGuess.getTime() - timezoneOffsetMilliseconds(utcGuess, timezone));
    return new Date(utcGuess.getTime() - timezoneOffsetMilliseconds(firstPass, timezone));
}
...
async function reconcileWorkDay(user, at = new Date()) {
    ...
    `;
        ...
        if (active.workDate > workDate) {
            const elapsed = Math.max(0, Math.floor((at.getTime() - active.activeStartedAt.getTime()) / 1000));
        ...
        while (segmentDate < workDate) {
            ...
            const elapsed = Math.max(0, Math.floor((boundary.getTime() - segmentStart.getTime()) / 1000));
...
function refreshCookieName(client) {
...
function setRefreshCookie(reply, refreshToken, client) {
...
function totalSeconds(workDay) {
    ...
    const activeSeconds = Math.max(0, Math.floor((Date.now() - workDay.activeStartedAt.getTime()) / 1000));
...
function screenshotPreviewUrl(key) {
...
app.post("/auth/refresh", async (request, reply) => {
    const body = z
        ...
        .parse(request.body ?? {});
    const refreshToken = body.refreshToken ?? request.cookies[refreshCookieName(body.client)];
    if (!refreshToken) {
    ...
    return reply.send(authResponse(storedToken.user, accessToken, nextRefreshToken));
});
...
app.get("/timer/today", { preHandler: app.authenticate }, async (request) => {
    const user = await prisma.user.findUniqueOrThrow({
        where: { id: request.user.userId },
    });
    return reconcileWorkDay(user);
});
app.post("/timer/start", { preHandler: app.authenticate }, async (request) => {
    const body = z
        .object({
...
app.post("/timer/stop", { preHandler: app.authenticate }, async (request) => {
    const body = z
        .object({
        occurredAt: z.string().datetime({ offset: true }).optional(),
    })
    ...
    return prisma.$transaction(async (tx) => {
      ...
      `;
        ...
        const elapsed = Math.max(0, Math.floor((occurredAt.getTime() - workDay.activeStartedAt.getTime()) / 1000));
        return tx.workDay.update({
            where: { id: workDay.id },
            data: {
                accumulatedSeconds: {
...
app.get("/me/work-days", { preHandler: app.authenticate }, async (request) => {
    const query = z
        .object({
...
app.put("/me/timezone", { preHandler: app.authenticate }, async (request) => {
    const body = z
        .object({
...
app.get("/me/screenshots", { preHandler: app.authenticate }, async (request) => {
...
app.post("/me/screenshots", {
    preHandler: app.authenticate,
    config: { bodyLimit: 10 * 1024 * 1024 },
}, async (request, reply) => {
    if (!s3 || !screenshotsBucket) {
        throw app.httpErrors.failedDependency("Screenshot storage is not configured on the server");
    }
    const body = z
        .object({
...
app.post("/admin/users", { preHandler: authorizeAdmin }, async (request, reply) => {
    const body = z
        .object({
        email: z.string().email(),
...
app.get("/admin/users/:id/time", { preHandler: authorizeAdmin }, async (request) => {
...
app.get("/admin/users/:id/screenshots", { preHandler: authorizeAdmin }, async (request) => {
    const params = z.object({ id: z.string().uuid() }).parse(request.params);
    const query = screenshotQuerySchema.parse(request.query);
    const before = query.before ? new Date(query.before) : null;
    const screenshots = await prisma.screenshot.findMany({
